...
appCommonDirectiveModule.constant('Constant',{
    url: "http://localhost:8001/serviceManager"//后台访问路径
});

//分页
appCommonDirectiveModule.directive('tmPagination',[function($scope){
    return {
        restrict: 'EA',
        template: '<div class="page-list">' +
            '共<strong>{{ conf.totalItems }}</strong>行, ' +
            '每页<strong>{{ conf.itemsPerPage }}</strong>行,' +
            '第<strong>{{ conf.currentPage }}/{{conf.totalItems/conf.itemsPerPage}}</strong>页 ' +
            '首页&nbsp;&nbsp;上一页&nbsp;&nbsp;<a href="#">下一页</a>&nbsp;&nbsp;<a href="#">末页</a>&nbsp;&nbsp;'+
            '<input type="text"  style="width: 30px" ng-model="jumpPageNum" ng-keyup="jumpPageKeyUp($event)"/>&nbsp;转到' +
